...
| ISSUE-004 | Recorder service XPC skeleton | OPEN | Create `native/mac-recorder-service` with XPC listener, `ping`, and `getPermissions`. | ISSUE-001 | XPC service builds and responds to `ping` and `getPermissions`. |
| ISSUE-005 | Event tap capture | OPEN | Implement Quartz event tap to emit key/mouse events via XPC. | ISSUE-004 | Key and mouse events are received by the Rust side as recorder events. |
